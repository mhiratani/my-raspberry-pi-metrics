...
# 現在のユーザー名を取得
CURRENT_USER=$(whoami)
METRICS_NAME="network-speed-monitor"
...
Description=Raspberry Pi Metrics About Network Speed NewRelic
After=network.target

[Service]
ExecStart=$VENV_DIR/bin/python $PROJECT_DIR/speedtest_monitor.py
...
# systemdタイマーファイルの内容
TIMER_CONTENT="[Unit]
Description=Raspberry Pi Metrics About Network Speed NewRelic Timer
...
# スクリプトに実行権限を付与
chmod +x "$PROJECT_DIR/speedtest_monitor.py"
